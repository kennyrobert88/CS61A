...
def picky_piggy(score):
    ...
    if(score == 0):
        ...
        if (score % 12 == 0):
            return 7
        score =  score - int(score / 12) * 12 
            
    return int(((10**(score)) / 7) % 10)    
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal:
        if(who == 0):
            ...
            score0 += take_turn(s0, score1, dice, goal)
            score0 += hog_pile(score0, score1)
        else:
            s1 = strategy1(score1, score0)
            score1 += take_turn(s1, score0, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def new(s0, s1, last = last_score, run = running_high):
        if who == 0:
            stan = s0 - last
            last = s0
        else:
        ...
        if (stan > run):
            run = stan
            print(stan,  "point(s)! That's a record gain for Player", str(who) + "!")
        return announce_highest(who, last, run)
    return new
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def averaged(*args, count = trials_count):
        i, total = 0, 0
        while i < count:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while count < 10:
        max_average = average(count + 1, dice)
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    ns = picky_piggy(opponent_score) + score
    if opponent_score > score and hog_pile(score, opponent_score) != 0:
        return 0
    elif picky_piggy(opponent_score) >= cutoff or (hog_pile(ns, opponent_score) and ns <= opponent_score):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11

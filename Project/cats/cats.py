...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        lst.append([i, diff_function(typed_word, i, limit)])
        if typed_word == i:
            return typed_word
    return(min(lst, key =lambda scores: min(scores[1:]))[0] if min(lst, key =lambda scores: min(scores[1:]))[1] <= limit else typed_word)
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    if start == goal:
        ...
        return 1 + feline_flips(start[1:], goal[1:], limit)
    return 0
    # END PROBLEM 6
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in sofar:
        ...
        c += 1
    upload({ 'id' : user_id, 'progress' :  c / len(prompt)})
    return c / len(prompt)
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            lst.append(times_per_player[i][j+1] - times_per_player[i][j])
        total.append(lst)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    total = [ [] for pi in player_indices]
    for wi in word_indices:
        fastest_player = 0
        fastest_time = time(match, fastest_player, wi)

        for pi in player_indices:
            player_time = time(match, pi, wi)
            if player_time < fastest_time:
                fastest_player = pi
                fastest_time = player_time
        total[fastest_player].append(word_at(match, wi))
    return total
    # END PROBLEM 10

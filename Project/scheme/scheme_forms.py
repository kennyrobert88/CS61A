...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        v = scheme_eval(expressions.rest.first, env)
        env.define(signature, v)
        return signature
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        # define enviroment using LambdaProcedure
        s, formals = signature.first, signature.rest
        val = do_lambda_form(Pair(formals, expressions.rest), env)
        env.define(s, val)
        return s
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while validate_bindings is not nil:
        pair = validate_bindings.first
        validate_form(pair, 2, 2)
        validate_bindings = validate_bindings.rest
